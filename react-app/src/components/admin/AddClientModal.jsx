                                    ...
                                    <div key={index} className="p-5 bg-gray-50 rounded-2xl border border-gray-200 transition-all hover:shadow-md">
                                        ...
                                        <div className="grid md:grid-cols-2 gap-3 mb-3">
                                            <input
                                                ...
                                                onBlur={(e) => {
                                                    // Auto-generate AI image if name exists and image is empty (or just prompt to regenerate)
                                                    if (e.target.value.trim() && !service.image) {
                                                        const seed = Math.floor(Math.random() * 1000);
                                                        const aiUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(e.target.value + " service professional high quality realistic")}&seed=${seed}`;
                                                        handleServiceChange(index, 'image', aiUrl);
                                                    }
                                                }}
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-xs font-semibold text-gray-500 flex justify-between">
                                                <span>Service Photo (AI Generated)</span>
                                                {service.image && (
                                                    <button
                                                        type="button"
                                                        onClick={() => {
                                                            const seed = Math.floor(Math.random() * 1000);
                                                            const aiUrl = `https://image.pollinations.ai/prompt/${encodeURIComponent(service.name + " service professional high quality realistic")}&seed=${seed}`;
                                                            handleServiceChange(index, 'image', aiUrl);
                                                        }}
                                            ...
                                            <div className="flex gap-2">
                                                {/* Hidden input for URL backing */}
                                                <input
                                                    type="hidden"
                                                    value={service.image}
                                                />

                                                {/* Preview Box is the main UI now */}
                                                <div className="relative w-full h-32 bg-gray-100 rounded-xl overflow-hidden border border-gray-200 group">
                                                    {service.image ? (
                                                        <img
                                                            ...
                                                            onError={(e) => {
                                                                e.target.onerror = null;
                                                                e.target.src = 'https://placehold.co/600x400/f3f4f6/9ca3af?text=Generating...';
                                                            }}
                                                        />
                                                    ) : (
                                                        <div className="flex items-center justify-center h-full text-gray-400 text-sm">
                                                            <SparklesIcon className="w-5 h-5 mr-2" />
                                                            Enter name to generate image
                                                        </div>
                                                    )}

                                                    {/* Upload Overlay */}
                                                    <div className="absolute inset-0 bg-black/50 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                                                        <label className="cursor-pointer bg-white text-gray-800 px-3 py-1.5 rounded-lg text-xs font-bold hover:bg-gray-100 transition-colors shadow-lg">
                                                            Replace Photo
                                                            <input
                                                                type="file"
                                                                accept="image/*"
